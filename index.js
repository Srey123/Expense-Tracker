...
function add_expense() {
  ...
  var category = document.getElementById("category").selectedIndex;//made change, wrong element was sele
  ...
  if (amount && date && note && selected) {
    ...
    new_row.innerHTML =
      `<tr style = border-bottom: 1px solid white;> <td>${amount}</td> <td>${selected}</td> <td>${date}</td> <td>${note}</td> </tr>`;
